...
int main()
{
	const int a = 10;    // 被const修饰的变量，在C++中已经是一个常量了，而且在定义时候必须对其进行初始化
...
public:
	Date(int year, int month, int day)
		: _year(year)
		, _month(_day)   // 注意：在_day初始化_month时，_day还没有初始化，即暂时还是随机值，因此_day初始化_month,_month中肯定就是随机值
		, _day(day)
	{
		cout << "Date(int,int,int):" << this << endl;
	}
...
提示：
用来统计对象个数的计数：应该要与对象没有任何关系，需要一个共享的计数
